    ...
    public static partial class UniGif
    {
            ...
            for (int x = 0; x < tex.width; x++)
            {
                ...
                if (dataIndex >= decodedData.Length)
                {
                    if (filledTexture == false)
                    {
                        ...
                        if (dataIndex == decodedData.Length)
                        {
                            LogManager.Error("dataIndex exceeded the size of decodedData. dataIndex:" + dataIndex + " decodedData.Length:" + decodedData.Length + " y:" + y + " x:" + x);
                        }
                    }
                    dataIndex++;
                    continue;
                ...
                // Get pixel color from color table
                {
                    ...
                    if (colorTable == null || colorTable.Count <= colorIndex)
                    {
                        if (filledTexture == false)
                        {
                            ...
                            if (colorTable == null)
                            {
                                LogManager.Error("colorIndex exceeded the size of colorTable. colorTable is null. colorIndex:" + colorIndex);
                            }
                            else
                            {
                                LogManager.Error("colorIndex exceeded the size of colorTable. colorTable.Count:" + colorTable.Count + " colorIndex:" + colorIndex);
                            }
                        }
                        dataIndex++;
                        continue;
        ...
        private static byte[] DecodeGifLZW(List<byte> compData, int lzwMinimumCodeSize, int needDataSize)
        {
            ...
            while (bitDataIndex < bitData.Length)
            {
                ...
                {
                    // Exit
                    LogManager.Warn("early stop code. bitDataIndex:" + bitDataIndex + " lzwCodeSize:" + lzwCodeSize + " key:" + key + " dic.Count:" + dic.Count);
                    break;
                }
                ...
                else if (key >= dic.Count)
                {
                    ...
                    {
                        LogManager.Warn("It is strange that come here. bitDataIndex:" + bitDataIndex + " lzwCodeSize:" + lzwCodeSize + " key:" + key + " dic.Count:" + dic.Count);
                        bitDataIndex += lzwCodeSize;
                        continue;
                    }
                }
