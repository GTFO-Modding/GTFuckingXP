...
namespace UniGif
{
    public static partial class UniGif
    {
        ...
        private static bool SetGifData(byte[] gifBytes, ref GifData gifData, bool debugLog)
        {
            if (debugLog)
            {
                LogManager.Debug("SetGifData Start.");
            }

            if (gifBytes == null || gifBytes.Length <= 0)
            {
                LogManager.Error("bytes is nothing.");
                return false;
            }
            ...
            {
                LogManager.Error("GIF header set error.");
                return false;
            }

            if (SetGifBlock(gifBytes, ref byteIndex, ref gifData) == false)
            {
                LogManager.Error("GIF block set error.");
                return false;
            }
            ...
            if (debugLog)
            {
                gifData.Dump();
                LogManager.Error("GIF block set error.");
                LogManager.Debug("SetGifData Finish.");
            }
            return true;
        }
        ...
        private static bool SetGifHeader(byte[] gifBytes, ref int byteIndex, ref GifData gifData)
        {
            ...
            {
                LogManager.Error("This is not GIF image.");
                return false;
            }
            ...
            {
                LogManager.Error("GIF version error.\nSupported only GIF87a or GIF89a.");
                return false;
            }
        ...
        private static bool SetGifBlock(byte[] gifBytes, ref int byteIndex, ref GifData gifData)
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    {
                        LogManager.Error("Infinite loop error.");
                        return false;
                    }
